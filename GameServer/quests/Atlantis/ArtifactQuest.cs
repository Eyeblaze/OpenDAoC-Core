...
namespace DOL.GS.Quests.Atlantis
{
    ...
    public class ArtifactQuest : BaseQuest
    {
        ...
        // einmalige Turn-In-Initialisierung beim Boot
        private static int _turnInInitGuard = 0;
        private static void EnsureTurnInAtBoot()
        {
            if (System.Threading.Interlocked.Exchange(ref _turnInInitGuard, 1) == 0)
            {
                try { ArtifactTurnInQuest.EnsureInitialized(); }
                catch { /* bewusst schlucken – idempotent und billig */ }
            }
        }
        ...
        public static void Init(string artifactID, Type questType)
        {
            ...
            // Safety: sicherstellen, dass DB-Daten (Artifacts/Scholars) geladen sind
            DOL.GS.ArtifactMgr.EnsureLoaded();
            ...
            if (scholars != null)
            {
                ...
                foreach (string scholar in scholars)
                {
                    ...
                    switch (realm)
                    {
                        case eRealm.Albion:
                            title = "Scholar";
                            npcs = WorldMgr.GetNPCsByName($"{title} {scholar}", realm);
                            break;
                    ...
                    if (npcs.Length == 0)
                    {
                        log.Warn(string.Format("ARTIFACTQUEST: {0} {1} not found in {2} for artifact {3}",
                            title, scholar, GlobalConstants.RealmToName(realm), artifactID));
                    }
                    else
                    {
                        npcs[0].AddQuestToGive(questType);
                        DOL.Logging.LoggerManager.Create(typeof(ArtifactQuest))
                            .Info($"AddQuestToGive: {artifactID} -> NPC '{npcs[0].Name}' (Realm {npcs[0].Realm}, Region {npcs[0].CurrentRegionID})");
                    }
